...
- **Publish a SURFACE MANIFEST (rouxte-web#21, contract carved in the
  canonical `PIPELINE-ROLES.md`).** Rouxte's is `surface.json` at the repo
  root, generated by `scripts/generate-surface.mjs` off the App Router tree
  and regenerated on every build (`prebuild`) — never hand-written. Served
  at `GET /api/ops/surface`; auth is `Bearer $CRON_SECRET`,
  `X-Answers-Secret: $ANSWERS_BUILD_SECRET` (the spine already holds it),
  or a super-admin session. Two rules ride with it: answer questions about
  Rouxte's surface by READING it, and cite the other seat's manifest for
  any claim about their surface — otherwise mark it UNMEASURED. "Absent,
  not zero."
