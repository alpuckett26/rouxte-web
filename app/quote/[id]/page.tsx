...
export default async function QuotePage({ params }: Params) {
  ...
  const lines = (quote.quote_lines ?? []).sort((a: { line_number: number }, b: { line_number: number }) => a.line_number - b.line_number);
              ...
              <div key={line.line_number} className="rounded-xl border border-gray-100 p-4">
                <div className="flex items-center justify-between mb-3">
                  <div>
                    ...
                    <p className="text-xs text-gray-400">{PLAN_LABELS[line.plan_type] ?? line.plan_type}</p>
                  </div>
...
export async function generateMetadata({ params }: Params) {
  const { id } = await params;
  const admin = createAdminClient();
  const { data } = await admin.from("quotes").select("customer_name").eq("id", id).single();
  return {
    title: data?.customer_name ? `Quote for ${data.customer_name} | Rouxte` : "AT&T Wireless Quote | Rouxte",
  };
}
